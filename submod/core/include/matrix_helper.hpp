...
class MatrixHelper {
 public:
  ...
  static bool write_matrix_to_CSV(const T& matrix,
  ...
  template <size_t n>
  static gds::core::ESM init_tridiagonal_matrix(
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (i < n - 1)
        entities.push_back(Eigen::Triplet<double>(i, i + 1, elements[2]));
      if (i > 0)
        entities.push_back(Eigen::Triplet<double>(i, i - 1, elements[0]));
    }
  ...
  template <size_t n>
  static gds::core::ESM init_fivediagonal_matrices(
      std::array<double, 5u> elements)
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (i > 0)
        entities.push_back(Eigen::Triplet<double>(i, i - 1, elements[1]));
      if (i >= std::sqrt(n) - 1)
        entities.push_back(
            Eigen::Triplet<double>(i, i - std::sqrt(n) + 1, elements[0]));
      if (i < n - 1)
        entities.push_back(Eigen::Triplet<double>(i, i + 1, elements[3]));
      if (i < n - std::sqrt(n) + 1)
        entities.push_back(
            Eigen::Triplet<double>(i, i + std::sqrt(n) - 1, elements[4]));
    }
